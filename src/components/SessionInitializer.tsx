...
const SessionInitializer: React.FC<SessionInitializerProps> = ({ cartId }) => {  const [qrToken, setQrToken] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (isExpired) {
      console.log('QR token expired, closing SSE connection and clearing error');
      if (sseRef.current) {
  ...
  useEffect(() => {
    ...
    const resetTimer = setTimeout(() => {
      console.log('Auto-resetting to default state after 1 minute of QR expiry');
      setQrToken(null);
      setIsExpired(false);
      setError('');
  ...
  useEffect(() => {
    ...
    sse.onmessage = (event) => {
      ...
      if (isExpired) {
        console.log('SSE error occurred but QR is expired - closing connection');
        sse.close();
          ...
          <div className="flex-1 p-6 lg:p-8 flex flex-col justify-between overflow-y-auto">
            ...
            <div>
              ...
              <div className="bg-blue-50 rounded-xl p-4 lg:p-6 mb-6">
                <h2 className="text-lg lg:text-xl font-semibold text-blue-800 mb-4">Quick Setup Instructions</h2>
                <div className="space-y-3">
                  <div className="flex items-start space-x-3">
                    <div className="w-6 h-6 lg:w-8 lg:h-8 bg-blue-500 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-white text-xs lg:text-sm font-bold">1</span>
                    </div>
                    <p className="text-blue-700 text-base lg:text-lg">Download the <strong>DuckyCart</strong> mobile app from your app store</p>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="w-6 h-6 lg:w-8 lg:h-8 bg-blue-500 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-white text-xs lg:text-sm font-bold">2</span>
                    </div>
                    <p className="text-blue-700 text-base lg:text-lg">Open the app and tap <strong>"Scan Cart"</strong></p>
                  </div>
                  <div className="flex items-start space-x-3">
                    <div className="w-6 h-6 lg:w-8 lg:h-8 bg-blue-500 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                      <span className="text-white text-xs lg:text-sm font-bold">3</span>
                    </div>
                    <p className="text-blue-700 text-base lg:text-lg">Scan the QR code on the right to start shopping</p>
                  </div>
                </div>
              </div>
              ...
              {/* App Store Links */}
              <div className="mb-4">
                <p className="text-gray-600 text-base lg:text-lg mb-3">Don't have the app yet?</p>
              ...
              <div className="text-center space-y-8">
                <div className="w-24 h-24 bg-gray-100 rounded-full flex items-center justify-center mx-auto">
                  <svg className="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20H4a2 2 0 01-2-2V6a2 2 0 012-2h16a2 2 0 012 2v12a2 2 0 01-2 2z" />
                  </svg>
                </div>
                <div>
                  ...
                  >
                    <QRCode 
                      ...
                      style={{ 
                        ...
                        maxWidth: "100%", 
                        width: "100%",
                        filter: isExpired ? 'grayscale(100%) opacity(0.3)' : 'none'
                      }}
                    />
                    {isExpired && (
                      <div className="absolute inset-0 flex items-center justify-center bg-red-500 bg-opacity-95 rounded-2xl cursor-pointer hover:bg-opacity-100 transition-all duration-200">
                        <div className="text-white text-center p-4">
                          ...
                          <p className="font-bold text-2xl mb-2">QR Code Expired!</p>
                          <p className="text-lg mb-4">This code is no longer valid</p>
                          <div className="bg-white bg-opacity-20 rounded-xl px-6 py-3 backdrop-blur-sm">
                            <p className="font-semibold text-lg">👆 Click to Generate New Code</p>
                          </div>
                        </div>
                      </div>
                    )}
                ...
                {isExpired ? (
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-xl p-4 max-w-sm mx-auto">
                    <div className="flex items-center justify-center space-x-2 text-green-600 mb-2">
                      ...
                      <p className="font-medium text-lg">Waiting for mobile app...</p>
                    </div>
